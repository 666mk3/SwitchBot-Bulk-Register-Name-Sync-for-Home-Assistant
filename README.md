...
- **一括登録**: 未登録のSwitchBot関連の統合フロー（Bluetooth検出など）が見つかった場合、自動的に登録処理を行い、名前を同期します。
...
| オプション名 | 必須 | 説明 |
| --- | --- | --- |
| `switchbot_token` | Yes | SwitchBotアプリから取得したトークン |
| `switchbot_secret` | Yes | SwitchBotアプリから取得したシークレット |
